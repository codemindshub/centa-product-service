// test/InventoryController.test.js
...
describe('InventoryController', () => {
  ...
  describe('GET /inventories/:id', () => {
    it('should retrieve the inventory document', async () => {
      const res = await chai.request(app)
  ...
  describe('GET /inventories', () => {
    it('should retrieve all inventories with pagination', async () => {
      const res = await chai.request(app)
        .get('/inventories')
        .set('x-token', token)
        .query({ page: 0 });
  ...
  describe('PUT /inventories/:id/delete', () => {
    it('should set is_deleted to true', async () => {
      const res = await chai.request(app)
        .put(`/inventories/${inventoryId}/delete`)
        .set('x-token', token);
      expect(res).to.have.status(200);
  ...
  describe('PUT /inventories/:id/restore', () => {
    it('should set is_deleted to false', async () => {
      const res = await chai.request(app)
        .put(`/inventories/${inventoryId}/restore`)
        .set('x-token', token);
      expect(res).to.have.status(200);
